...
import { kidHoodiesQuery } from '@/sanity/lib/query';
...
const ShopPage = () => {
  ...
  const hardcodedLinks = [
    '/product25', '/product43', '/product41', '/product38',
    '/product24', '/product12',
  ];

  const fetchData = useCallback(async () => {
    try {
      let query = kidHoodiesQuery;
      const params: QueryParams = {};

      if (searchTerm) {
        query = `*[_type == "product" && category == "kids" && subcategory == "hoodies" && lower(title) match lower($searchTerm)]`;
  ...
  const debouncedFetch = React.useMemo(() => debounce(() => fetchData(), 300), [fetchData]);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex justify-start w-32 sm:w-40 relative">
            <input
              ...
              placeholder="Search kids hoodies..."
          ...
          <div className="text-center flex-1">
            ...
            <p className="text-gray-500 mt-2 text-sm md:text-base">Discover the latest kids hoodies at unbeatable prices.</p>
          </div>
