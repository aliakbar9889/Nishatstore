...
import { WomenHoodiesQuery } from '@/sanity/lib/query';
...
const ShopPage = () => {
  ...
  const hardcodedLinks = [
    '/product17', '/product8', '/product7', '/product1', '/product9',
  ];

  const fetchData = useCallback(async () => {
    try {
      let query = WomenHoodiesQuery;
      const params: QueryParams = {};

      if (searchTerm) {
        query = `*[_type == "product" && category == "women" && subcategory == "hoodies" && lower(title) match lower($searchTerm)]`;
  ...
  // Use debounce *inside* useEffect to avoid React warning
  useEffect(() => {
  ...
  const handleSearchIconClick = () => {
    fetchData(); // Immediate fetch on search icon click
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex justify-start w-32 sm:w-40 relative">
            <input
              ...
              placeholder="Search womens hoodies..."
          ...
          <div className="text-center flex-1">
            ...
            <p className="text-gray-500 mt-2 text-sm md:text-base">
              Discover the latest womens hoodies at unbeatable prices.
            </p>
          </div>
              ...
              >
                ...
                >
                  <Image
                    src={
                      product.image
                        ? urlFor(product.image as SanityImageSource).url()
