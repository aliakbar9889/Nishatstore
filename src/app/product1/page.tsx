...
export default function ProductPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const products: Product[] = await client.fetch(`*[_type == "product"]{_id, title, price, discount, image}`);
        setProduct(products[0]); // Fetching first product
      } catch (error) {
  ...
  // Reusing addToLocalStorage from ShopPage
  ...
  if (!product) return <p className="text-center mt-20 text-lg text-gray-600">Loading...</p>;
