...
import { kidProductsQuery } from '@/sanity/lib/query';
...
const ShopPage = () => {
  ...
  const hardcodedLinks = [
    '/product39', '/product40', '/product44', '/product27',
    '/product14', '/product26', '/product45',
  ];

  const fetchData = useCallback(async () => {
    try {
      let query = kidProductsQuery;
      const params: QueryParams = {};

      if (searchTerm) {
        query = `*[_type == "product" && category == "kids" && title match $searchTerm]`;
  ...
  }, [searchTerm, sortOption, debouncedFetch]);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="text-center flex-1">
            ...
            <p className="text-gray-500 mt-2 text-sm md:text-base">
              Discover the latest kids products at unbeatable prices.
            </p>
          </div>
        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {products.map((product, index) => (
              <div key={product._id} className="border rounded-lg overflow-hidden shadow-sm group relative bg-white">
                <Link href={hardcodedLinks[index] || '#'} className="relative w-full h-[300px] overflow-hidden block">
                  <Image
